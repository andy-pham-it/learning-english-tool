...
* **Kiến trúc:** Angular, Tailwind CSS.  
...
| **Tích hợp Code Snippet** | Sử dụng thư viện syntax highlighting để hiển thị các đoạn code Angular/TypeScript đi kèm với mỗi mẫu câu. |
...
`[`  
  `{`  
    `"id": 1,`  
    `"category": "Setup",`  
    `"english": "Let's start by defining the interface for our component's inputs to ensure type safety.",`  
    `"vietnamese": "Hãy bắt đầu bằng việc định nghĩa interface cho input của component để đảm bảo an toàn về kiểu dữ liệu.",`  
    `"code": "export interface UserCardInput { id: string; username: string; }",`  
